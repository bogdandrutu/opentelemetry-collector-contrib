...
// BorrowContext returns a new TransformContext with the provided parameters from a pool of contexts.
// Caller must call ReturnContext.
func BorrowContext(span ptrace.Span, instrumentationScope pcommon.InstrumentationScope, resource pcommon.Resource, scopeSpans ptrace.ScopeSpans, resourceSpans ptrace.ResourceSpans, options ...TransformContextOption) *TransformContext {
...
// ReturnContext returns the TransformContext to the pool of contexts.
func ReturnContext(tCtx *TransformContext) {
	tCtx.cache.Clear()
	tcPool.Put(tCtx)
}
